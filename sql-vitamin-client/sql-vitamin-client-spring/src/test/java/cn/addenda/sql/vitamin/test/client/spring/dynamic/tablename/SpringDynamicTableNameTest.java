package cn.addenda.sql.vitamin.test.client.spring.dynamic.tablename;
...
import cn.addenda.sql.vitamin.rewriter.dynamic.condition.DynamicConditionUtils;
...
import cn.addenda.sql.vitamin.rewriter.dynamic.tablename.DynamicTableNameUtils;
 ...
 */
public class SpringDynamicTableNameTest {
  ...
  public static void before() {
    ...
    context.register(DynamicTableNameTestConfiguration.class);
    context.refresh();
  ...
  private void updateUserFail(int id, String name) {
    DynamicTableNameUtils.rename("users1", "users", () -> {
      DynamicConditionUtils.dynamicCondition(
          DynamicConditionConfigBatch.of(
              ArrayUtils.asArrayList(DynamicConditionConfig.of(
                  DynamicConditionOperation.TABLE_ADD_JOIN_CONDITION, "users1", "if_del = 1"))),
          new Runnable() {
            @Override
            @SneakyThrows
            public void run() {
              try (Connection connection = dataSource.getConnection();
                   PreparedStatement preparedStatement = connection.prepareStatement("update users1 set name = ? where id = ?")) {
  ...
  private void delete(int id) {
    DynamicTableNameUtils.rename("users1", "users", new Runnable() {
      @Override
      @SneakyThrows
      public void run() {
        try (Connection connection = dataSource.getConnection();
    ...
    return DynamicTableNameUtils.rename("users1", "users", () -> {
      ...
      return TombstoneUtils.tombstone(TombstoneConfig.of(true, null, null, null),
          new Supplier<User>() {
            ...
            public User get() {
              try (Connection connection = dataSource.getConnection();
                   PreparedStatement preparedStatement = connection.prepareStatement("SELECT id, name, if_del FROM users1");
    ...
    DynamicTableNameUtils.rename("users1", "users", () -> {
      DynamicItemUtils.dynamicItem(DynamicItemConfigBatch.of(
          ArrayUtils.asArrayList(DynamicItemConfig.of(
              DynamicItemOperation.INSERT_ADD_ITEM, "users1", itemName, itemValue))
      ), () -> {
        DefaultBaseEntitySource.execute("aaa", new Runnable() {
          ...
          public void run() {
            try (Connection connection = dataSource.getConnection();
                 PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO users1 (id, name) VALUES (?, ?)")) {
              preparedStatement.setInt(1, id);
  ...
  public void updateUser(int id, String name, String itemName, String itemValue) {
    ...
    DynamicTableNameUtils.rename("users1", "users", () -> {
      DynamicItemUtils.dynamicItem(DynamicItemConfigBatch.of(
          ArrayUtils.asArrayList(DynamicItemConfig.of(
              DynamicItemOperation.UPDATE_ADD_ITEM, "users1", itemName, itemValue))
      ), new Runnable() {
        @Override
        @SneakyThrows
        public void run() {
  ...
  public User getAllUsers() {
    return DynamicTableNameUtils.rename("users1", "users", new Supplier<User>() {
      @Override
      @SneakyThrows
      public User get() {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement("SELECT id, name, if_del, aaa FROM users1");
             ResultSet resultSet = preparedStatement.executeQuery()) {
